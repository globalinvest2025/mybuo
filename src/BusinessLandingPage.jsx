// src/BusinessLandingPage.jsx (Versión Final con SVG Corregido)

import React from 'react';
import { useNavigate } from 'react-router-dom';
import { Camera, Sparkles, MapPin, ArrowRight, CheckCircle } from 'lucide-react';

export default function BusinessLandingPage() {
  const navigate = useNavigate();

  const handleLoginWithGoogle = async () => {
    alert('Simulating Login with Google...');
    try {
      const userResponse = await fetch('http://localhost:3001/users/1122334455');
      if (!userResponse.ok) throw new Error("User not found");
      const loggedInUser = await userResponse.json();

      if (loggedInUser) {
        alert(`¡Welcome, ${loggedInUser.name}! We are redirecting you to your dashboard.`);
        navigate('/dashboard');
      }
    } catch (error) {
      alert(`Error: Could not connect to the server. Is 'npm run server' active in another terminal? Details: ${error.message}`);
    }
  };
    ...
    <main>
      <section className="text-center py-20 px-6 bg-gradient-to-br from-white via-purple-50 to-blue-50">
        <h1 className="text-5xl md:text-6xl font-extrabold text-gray-900 leading-tight">
          Put Your Tacoma Business on the Map.
          <br />
        ...
        <p className="max-w-3xl mx-auto mt-6 text-lg text-gray-600">
          Join Tacoma's fastest-growing business directory. Create a professional profile in minutes and connect with thousands of local customers looking for you. No subscriptions, no hidden fees.
        </p>
        ...
        >
          {/* SVG del logo de Google - VERSIÓN COMPLETA Y CORREGIDA */}
      ...
      {/* El resto de tu landing page (secciones de Features, Services, etc.) */}
      <section className="py-20 px-6 bg-white">
        ...
        {/* Pega esta sección de nuevo en tu archivo si la quieres */}
